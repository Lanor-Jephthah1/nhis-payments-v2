...
- **Key Features:**
  ...
  - **Fail-Safe Checkpoints:** Saves progress to `nhis_checkpoint_v2.txt`. If a fast-forward jump is intercepted by the server, it instantly raises a fatal exception to abort the process rather than silently overwriting the checkpoint and corrupting the database.
...
python main.py
```
This will launch the scraper in headless mode, generate `nhis_payments_v2.csv`, and track progress in `nhis_checkpoint_v2.txt`.
